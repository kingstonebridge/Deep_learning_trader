...
class DeepLearningTrader:
    ...
    def create_cnn_lstm_model(self, input_shape):
        ...
        model = Sequential([
            ...
            Dropout(0.3),
            Dense(32, activation='relu'),
            Dense(2, activation='softmax')  # 2 classes: UP, DOWN
        ])
    ...
    def prepare_deep_learning_data(self, features, lookback=30):
        ...
        for i in range(lookback, len(features)-1):
            ...
            future_return = (features[i+1, 0] > 0.002)  # 0.2% price increase
            if features[i+1, 0] > 0.005:  # Strong uptrend
                y.append([1, 0, 0])  # BUY
            elif features[i+1, 0] < -0.003:  # Strong downtrend
                y.append([0, 1, 0])  # SELL
            else:
                y.append([0, 0, 1])  # HOLD
    ...
    def deep_learning_prediction(self, coin_data):
        ...
        try:
            ...
            prices, volumes = self.get_historical_data(coin_id, 10)
            if len(prices) < 50:
                return None
...
def status():
    if trader:
        return {
            ...
            "win_rate": round(trader.performance['win_rate'] * 100, 1),
            "total_trades": trader.performance['total_trades'],
            "open_trades": len(trader.open_trades),
            "model_trained": trader.model_trained
        }
